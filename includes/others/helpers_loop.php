...
if ( ! function_exists( 'sp_get_asset' ) ) {
	 ...
	 * @param  string $include  If it is True, it will include not a link but the content of the file.
	 *
	 * @return string  Url asset
	 */
	function sp_get_asset( $asset, $include = false ) {
		$template_uri = get_template_directory_uri();

		if ( $include ) {
			return file_get_contents( "{$template_uri}/images/{$asset}" );
		} else {
			return esc_url( "{$template_uri}/images/{$asset}" );
		}
	}
}
