...
function scssSite() {
    return gulp.src('assets/source/scss/Site/*.*')
...
function scssBlocks() {
    return gulp.src('assets/source/scss/Blocks/*.*')
...
function scssComponents() {
    return gulp.src('assets/source/scss/Components/*.*')
...
function codeSvgs() {
    ...
    for (let index = 0; index < filesNamesFilter.length; index++) {
        ...
        //Read file content
        filesContents[index] = fs.readFileSync(config.pathIconsOrigin + '/' + element, {
            encoding: 'utf-8'
        });
...
.icon {
    ...
    fs.writeFileSync( config.pathExportIconsSheet + '_icons-sheet.scss', content);
    // console.log(content);
...
exports.default = function () {
    ...
    gulp.parallel(scssSite, iconSh);
    gulp.task(css);
    gulp.watch(['assets/**/*.scss', '!assets/**/_site.scss'], gulp.series(scssSite, css))
